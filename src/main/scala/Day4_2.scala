...
object Day4_2 {
  ...
  def fillGrids(
      ...
      lastWinningScore: Option[Int] = None
  ): IO[Int] = {
    grids match {
      case Nil => IO(lastWinningScore.get)
      case nonEmptyGrids =>
        nonEmptyGrids
          .collectFirst {
            case grid if grid.isComplete => grid
          } match {
          case Some(grid) =>
            val score =
              computeScore(lastPickedNumber = lastPickedNumber, winner = grid)
            fillGrids(
              grids = nonEmptyGrids.filterNot(_ == grid),
              randomNumbers = randomNumbers,
              lastPickedNumber = lastPickedNumber,
              lastWinningScore = Some(score)
            )
          case None =>
